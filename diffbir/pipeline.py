...
class Pipeline:
    ...
    def apply_upsample(
        ...
        """Bicubic upsample LQ to output size."""
        return F.interpolate(
            lq, size=self.output_size, mode="bicubic", antialias=True
        )
    ...
    def run(
        ...
        # Bicubic upsample LQ to target resolution as the condition for ControlNet
        cond_img = self.apply_upsample(lq_tensor)
        assert all(x >= 512 for x in cond_img.shape[2:]), (
            "The resolution of upsampled image should be >= 512, "
